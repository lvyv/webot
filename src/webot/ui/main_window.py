...
from ..agent import MainLoop
from ..skills import (
    click_ui_element, find_and_input_text,
    resize_window, move_window, activate_window,
...
class WebotWindow(QMainWindow):
    ...
    def _open_rules_file(self):
        ...
        if not os.path.exists(path):
            from ..skills.auto_reply_skill import save_default_rules
    ...
    def _agent_tick(self):
        ...
        unreads, processed, current = self.agent.tick()
        ...
        if current:
            self.lbl_agent_status.setText(f"状态: 运行中 - {current}")
        else:
            self.lbl_agent_status.setText("状态: 运行中")
